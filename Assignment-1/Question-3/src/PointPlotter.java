...
public class PointPlotter extends Applet implements ActionListener {
    ...
    public boolean drawHorizontalLine(Graphics g, int x1,int x2,int y,int originX,int originY,int gridGap,Color c,int diameter){
        if(x2<x1){// swap if x1>x2
            x1=x1^x2;
            x2=x1^x2;
            x1=x1^x2;
        }
        for(int i=x1;i<=x2;i++){
            plotPoint(g,originX,originY,gridGap,i,y,c,diameter,"square");
        }
        return true;
    }
    public boolean drawVerticalLine(Graphics g, int y1,int y2,int x,int originX,int originY,int gridGap,Color c,int diameter){
        if(y2<y1){// swap if y1>y2
            y1=y1^y2;
            y2=y1^y2;
            y1=y1^y2;
        }
        for(int i=y1;i<=y2;i++){
            plotPoint(g,originX,originY,gridGap,x,i,c,diameter,"square");
        }
        return true;
    }
    public boolean drawSquare(Graphics g,Color c,int []lowerLeft,int []upperRight,int diameter,int gridGap,int originX,int originY){
        drawHorizontalLine(g,lowerLeft[0],upperRight[0],lowerLeft[1],originX,originY,gridGap,c,diameter);
        drawHorizontalLine(g,lowerLeft[0],upperRight[0],upperRight[1],originX,originY,gridGap,c,diameter);
        drawVerticalLine(g,lowerLeft[1],upperRight[1],lowerLeft[0],originX,originY,gridGap,c,diameter);
        drawVerticalLine(g,lowerLeft[1],upperRight[1],upperRight[0],originX,originY,gridGap,c,diameter);
        return true;
    }
    public void paint(Graphics g) {
        ...
        //draws square
        drawSquare(g,Color.red,new int[]{-10,10},new int[]{10,-10},22,gridGap,originX,originY);
        drawSquare(g,Color.blue,new int[]{0,0},new int[]{15,15},22,gridGap,originX,originY);
    }
