...
public class Midpoint extends Applet implements ActionListener {
    ...
    public void drawLineMidPoint(Graphics g, int[] point1, int[] point2, int[] origin) {
        int dx = point2[0] - point1[0];
        int dy = point2[1] - point1[1];
        int d = dy - (dx / 2);
        int x = point1[0], y = point1[1];
        while (x < point2[0]) {
            plotPoint(g, origin, x, y, Color.blue);
            x++;
            if (d < 0) {
                d = d + dy;
            }
    ...
    public void paint(Graphics g) {
        ...
        // plot line
        drawLineMidPoint(g, endPoint1, endPoint2, new int[] { originX, originY });
    }
